...
This is a pure NumPy implementation — no PyTorch, no GPU required. A word-level Transformer trained on Chinese QA pairs, built on top of [`numpy-grad`](https://github.com/ryansoq/numpy-grad) — our own array-level autograd engine in NumPy.
...
```bash
# 1. Install numpy-grad (our autograd engine)
git clone https://github.com/ryansoq/numpy-grad
export PYTHONPATH=$PWD/numpy-grad:$PYTHONPATH
# or pip install -e numpy-grad
```
...
Word-level GPT-1 Mini, expressed as `numpy-grad` modules:

```
Embedding (token + positional, both learned)
  └─ TransformerBlock × N (pre-norm)
       ├─ LayerNorm
  ...
  └─ Linear out_proj → vocab logits
  └─ CrossEntropy loss
```

Forward composes Tensor expressions; backward is automatic via the `numpy-grad`
expression graph. AdamW + global gradient clipping + cosine LR with warmup.
